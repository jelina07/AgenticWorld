...
const NEXT_PUBLIC_PROOF_URL = isDev()
  ? "https://github.com/joshuamind/vana-satya-proof-template-py/releases/download/v12/my-proof-12.tar.gz"
  : "https://github.com/joshuamind/vana-satya-proof-template-py/releases/download/v14/my-proof-14.tar.gz";
...
export default function useVanaSend(options?: Options<string, any>) {
  ...
  const result = useRequest(
    async (walletAddress: string) => {
      ...
      try {
        console.log("HEAD response1:");

        const response = await fetch(`https://fcno.mindnetwork.xyz/${fileHash}`, { method: "get" });
        console.log("HEAD response2:", response.status);
      ...
      let file_Id, public_key;
      // 检查是否已经将文件注册到DataRegistry合约中
      file_Id = await getFileId(chainId as number, downloadUrl);
      ...
      if (!file_Id) {
        ...
        public_key = await getPublicKey(chainId as number);
      }

      setStep(2);
      await requestContributionProof(chainId as number, Number(file_Id) as number, signature, public_key);
      setStep(3);
      await claimReward(chainId as number, file_Id as number);
...
async function getFileId(chainId: number, url: string) {
  const dataRegistryAddress = getContractAddress(chainId, "DataRegistryProxy");
  const dataRegistryAbi = DataRegistryImplementationAbi;

  const fileId = (await readContract(config, {
...
async function getPublicKey(chainId: number) {
...
async function addFileToDataRegistry(walletAddress: string, downloadUrl: string, signature: string, chainId: number) {
  ...
  const gasEstimate1 = await estimateGasUtil(
    dataRegistryAbi,
  ...
  const tx = await writeContract(config, {
    ...
    gas: gasEstimate1 + gasEstimate1 / BigInt(3), // Add 33% buffer to gas estimate
  });
...
async function requestContributionProof(chainId: number, fileId: number, signature: string, publicKey: string) {
  ...
  const gasEstimate2 = await estimateGasUtil(teePoolAbi, "requestContributionProof", [fileId], teePoolAddress);
  ...
  const hash = await writeContract(config, {
    ...
    args: [fileId],
    gas: gasEstimate2 + gasEstimate2 / BigInt(3), // Add 33% buffer to gas estimate
  });
  console.log("hash11", hash);
...
async function claimReward(chainId: number, fileId: number) {
  const abi = DataLiquidityPoolImplementationAbi as any;
  const address = getContractAddress(chainId, "DataLiquidityPoolProxy");
  const gasEstimate3 = await estimateGasUtil(abi, "requestReward", [BigInt(fileId), BigInt(1)], address);
  const hash = await writeContract(config, {
    address,
    abi,
    functionName: "requestReward",
    args: [BigInt(fileId), BigInt(1)], // Convert both values to bigint
    gas: gasEstimate3 + gasEstimate3 / BigInt(3), // Add 33% buffer to gas estimate
  });
  console.log("claimReward hash:", hash);
