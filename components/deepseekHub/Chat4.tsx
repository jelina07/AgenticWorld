...
export default function Chat4() {
  ...
  const [bubbleThinkId, setBubbleThinkId] = useState<any>([]);
  console.log("bubbleThinkId", bubbleThinkId);

  const changeBubbleThinkId = (value: number | string) => {
    console.log("value", value);
    const newArray = bubbleThinkId.concat(value);
    setBubbleThinkId(newArray);
  };
  ...
  const [agent] = useXAgent({
    request: async (
      ...
      console.log("credti", credti, messages);
    ...
    ({ id, message, status }) => ({
      ...
      content:
        ...
        status === "aiThink" ? (
          <BubbleThink
            BubbleThinkKey={id}
            changeBubbleThinkId={changeBubbleThinkId}
            bubbleThinkId={bubbleThinkId}
          />
        ) : (
          <div
